...
#ifdef CHANGED
	Thread *ownerThread; //Thread which owns the lock
	List *queue;  	// queue of threads that are waiting on the lock
#endif
};
...
class Condition {
  ...
  private:
    ...
    // plus some other stuff you'll need to define
#ifdef CHANGED
    Lock* conditionLock;  //lock for the condition
    List *queue;     // queue of threads that are waiting on the condition
#endif
};
#endif // SYNCH_H
