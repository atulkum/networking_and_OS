...
void hex2int(char* src, unsigned char* dest, int len){ 
	char temp[3];
	temp[2] = '\0';
	int i;
	for (i=0; i<len; i+=2) {
...
*/
RouterNode* findRouterEntry(char* uoid){
...
*/
void writeLog(int dir, char* nodeid, int size, int ttl, int type, char* data, char *msguoid){	
	struct timezone tz;
	struct timeval tv;
	gettimeofday(&tv, &tz);		
	...
	fprintf(logfile, "%s %10ld.%03ld %s %s %d %d %s %s\n",  msgdir[dir], tv.tv_sec, tv.tv_usec/1000, nodeid, msgtype[type], size+27, ttl, toprint, data);
...
*/
void putNewRouteEntry(char* uoid, Node* ngb){
   //put the new uoid into the routing table
	...
	Append(&routerhead, &routertail, rnode);																						
...
*/
void forwardResponse( Message* msg, unsigned char* data, Node* ngbrtosend){
	...
	else if(response->type == SEARCHRESPONSE){
		char toprint[9];
		int2hex((unsigned char*)(response->data + 16), 4, toprint);
		writeLog(FORW, ngbrtosend->nodeId, response->dataLength, response->ttl, SHRS, toprint, response->uoid);
	}
...
*/
void forwardRequest( Message* msg, unsigned char* data, Node* ngbrtosend){
	Message *request = (Message*)malloc(sizeof(Message));
	if (request == NULL) {
		fprintf(logfile, "** malloc() failed\n");fflush(logfile);
	}				
	memcpy(request, msg, sizeof(Message));
	request->ttl--;
	request->data = (unsigned char*)malloc(msg->dataLength);
	memcpy(request->data, data, msg->dataLength);
	
	if(request->type == STATUS){
		if(request->data[0] == 1){
			writeLog(FORW, ngbrtosend->nodeId, request->dataLength, request->ttl, STRQ, "neighbors", request->uoid);
		}
		else{
			writeLog(FORW, ngbrtosend->nodeId, request->dataLength, request->ttl, STRQ, "files", request->uoid);
		}
	}
	if(request->type == SEARCH){
		char toprint[256];
		memset(toprint, 0, 256);
		sprintf(toprint, "%d %s", request->data[0], request->data + 1);		
		writeLog(FORW, ngbrtosend->nodeId, request->dataLength, request->ttl, SHRQ, toprint, request->uoid);
	}
	pthread_mutex_lock(&(ngbrtosend->nodeLock));				
	Append(&(ngbrtosend->writeQhead), &(ngbrtosend->writeQtail), request);
	ngbrtosend->qlen++;
	pthread_mutex_unlock(&(ngbrtosend->nodeLock));
	pthread_cond_signal(&(ngbrtosend->nodeCond));							
}
